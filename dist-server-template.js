...
app.use(express.static('.')); // Serve built files from current directory
app.use(express.static(__dirname)); // Serve static files from root

// Data directory - עצמאי לחלוטין
...
async function writeJsonFile(filePath, data) {
  try {
    ...
    console.log(`💾 Successfully saved data to ${path.basename(filePath)} (${jsonString.length} bytes)`);
...
async function startServer() {
  try {
    console.log('🔧 Initializing standalone DIST server...');
    ...
    app.listen(PORT, () => {
      ...
      console.log('💡 This is a completely independent DIST server');
    });
  } catch (error) {
